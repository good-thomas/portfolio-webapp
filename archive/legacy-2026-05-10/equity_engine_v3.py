...
# --- Effiziente Hilfsfunktionen ---

def calc_stats_fast(rets):
    if len(rets) < 2: return {"cagr": 0, "sharpe": 0}
    cum_ret = (1 + rets).prod()
    years = len(rets) / 12
    cagr = float(cum_ret**(1/years) - 1) if years > 0 else 0
    vola = float(rets.std() * math.sqrt(12))
    return {"cagr": cagr, "sharpe": cagr / vola if vola > 0 else 0}
...
def api_v3():
    try:
        ...
        if "XLC" in df and "XLK" in df: df["XLC"] = df["XLC"].fillna(df["XLK"] * (df["XLC"].dropna().iloc[0]/df["XLK"].loc[df["XLC"].dropna().index[0]]))
        
        # Mapping & Returns vorab berechnen (Vektorisierung!)
        ...
        # 2. Vorbereiten der Optimierungseinstellungen
        configs = [
        ...
        for i in range(start_i, len(prices)-1):
            # Optimierung nur wenn nötig (Thomas Regel: Jeden Monat wenn nicht ALPHA)
            if months_active >= z_lock or mode != "ALPHA":
                best_cfg, max_s = configs[0], -1
                window_rets = rets_df.iloc[i-x_win:i]
                
                for cfg in configs:
                    # Score-Berechnung vektoriell für das ganze Fenster (extrem schnell!)
                    # Hier vereinfacht für die Performance:
                    ...
                    # Simulierter Check (vereinfacht für Speed)
                    if s_score.max() > 0: best_cfg = cfg; break 
                curr_p = best_cfg
                months_active = 0

            # Signal Check für aktuellen Monat
            def get_s(col):
                p_vec = prices[col]
            ...
            if qual:
                ...
                for s in top: weights[s] = 1.0/len(top)
                mode = "ALPHA"
            elif eq_s > cash_s:
