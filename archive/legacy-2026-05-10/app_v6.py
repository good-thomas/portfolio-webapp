...
RISK_BRAKE_ITERATIONS = 12
RISK_BRAKE_REDUCTION = 0.85
...
def get_sector_buckets(sector_proxy_set):
    if sector_proxy_set not in SECTOR_PROXY_SETS:
        raise ValueError(
            "sector_proxy_set muss einer dieser Werte sein: "
            + ", ".join(sorted(ALLOWED_SECTOR_PROXY_SETS))
        )
...
def load_sg_trend_series(path=SG_TREND_FILE):
    ...
    sg["date"] = pd.to_datetime(
        sg["date"].astype(str), format="%Y%m%d", errors="coerce"
    )
...
def load_data(include_bitcoin=True, sector_proxy_set=DEFAULT_SECTOR_PROXY_SET):
    ...
    for bucket, ticker in gics_buckets.items():
        try:
            if ticker in data.columns:
                series[bucket] = pd.to_numeric(
                    data[ticker], errors="coerce"
                ).resample("ME").last()
        except Exception:
            continue
    ...
    for col in prices.columns:
        if col == RISK_FREE_ASSET:
            returns[col] = (
                pd.to_numeric(prices[col], errors="coerce") / 100.0
            ) / 12.0
        else:
            returns[col] = pd.to_numeric(
                prices[col], errors="coerce"
            ).pct_change()
...
def compute_benchmark_score(prices, i):
    ...
    return (
        BENCHMARK_WEIGHTS["equities"] * equity_score
        + BENCHMARK_WEIGHTS["bonds"] * bond_score
    )
...
def compute_blended_weights(
    ...
    benchmark_score,
    total_weight,
    power=EQUITY_ENGINE_EDGE_POWER,
):
    ...
    edges = {
        b: max(0.0, float(item_scores[b] - benchmark_score))
        for b in top_items
        if pd.notna(item_scores.get(b))
    }
    ...
    if power_sum <= 0:
        ...
        edge_weights = {
            b: total_weight * edge_power_scores[b] / power_sum
            for b in edge_power_scores
        }

    blended_weights = {}
    for b in top_items:
        blended_weights[b] = (
            EQUITY_ENGINE_EQUAL_WEIGHT_BLEND * equal_weights.get(b, 0.0)
            + EQUITY_ENGINE_EDGE_WEIGHT_BLEND * edge_weights.get(b, 0.0)
        )

    total = sum(blended_weights.values())
    if total > 0:
        blended_weights = {
            b: total_weight * w / total for b, w in blended_weights.items()
        }
...
def compute_equity_engine_weights(
    ...
    i,
    gics_buckets,
    factor_max=DEFAULT_EQUITY_FACTOR_MAX,
):
    equity_score = compute_score(prices, "equities", i)

    available_buckets = [
        b for b in gics_buckets if has_lookback(prices, b, i)
    ]

    bucket_scores = {
        b: compute_score(prices, b, i) for b in available_buckets
    }
    ...
    candidates = [
        ...
        and pd.notna(equity_score)
        and bucket_scores[b] > equity_score
        and bucket_scores[b] > 0
    ]
    ...
    sector_weights, equal_weights, edge_weights, edges, edge_power_scores = compute_blended_weights(
        top_buckets,
        bucket_scores,
        equity_score,
        factor_weight,
    ...
    diagnostics = {
        ...
        "edge_power_scores": {
            k: float(v) for k, v in edge_power_scores.items()
        },
        "internal_weights": {
            k: float(v) for k, v in internal_weights.items()
        },
    }
...
def compute_commodity_engine_weights(prices, i, commodity_buckets):
    available = [
        b for b in commodity_buckets if has_lookback(prices, b, i)
    ]

    raw_scores = {
        b: compute_score(prices, b, i) for b in available
    }
    ...
    for b, s in raw_scores.items():
        if pd.isna(s) or s <= 0:
            ...
            adjusted_scores[b] = float(s) * (
                BROAD_COMMODITY_SCORE_BOOST
                if b == "broad_commodities"
                else 1.0
            )

    power_scores = {
        b: s**COMMODITY_ENGINE_SCORE_POWER
        for b, s in adjusted_scores.items()
        if s > 0
    }
    ...
    caps = {
        b: MAX_SINGLE_COMMODITY_SUB / MAX_TOTAL_COMMODITIES
        for b in weights
    }
...
def build_benchmark_asset_weights():
    return {
        EQUITY_ENGINE_ASSET: BENCHMARK_WEIGHTS["equities"],
        "bonds": BENCHMARK_WEIGHTS["bonds"],
    }


def estimate_asset_level_vol(asset, rets, i, equity_engine_weights=None, commodity_engine_weights=None):
    start = max(0, i - 12)

    if asset == EQUITY_ENGINE_ASSET and equity_engine_weights:
        parts = []
        for inner_asset, inner_weight in equity_engine_weights.items():
            if inner_asset in rets.columns:
                parts.append(rets[inner_asset].iloc[start:i] * inner_weight)
        if parts:
            s = sum(parts)
            v = s.std()
            return float(v) if pd.notna(v) and v > 0 else 0.01

    if asset == COMMODITY_ENGINE_ASSET and commodity_engine_weights:
        parts = []
    ...
    if asset in rets.columns:
        v = rets[asset].iloc[start:i].std()
        return float(v) if pd.notna(v) and v > 0 else 0.01

    return 0.01


def apply_risk_contribution_brake(
    weights,
    rets,
    i,
    equity_engine_weights=None,
    commodity_engine_weights=None,
    max_rc=MAX_RISK_CONTRIBUTION,
):
    w = normalize_weights(weights.copy())

    if not w:
        return w, {}

    diagnostics = {
        "before_weights": {k: float(v) for k, v in w.items()},
        "iterations": [],
    }

    for _ in range(RISK_BRAKE_ITERATIONS):
        vols = {
            a: estimate_asset_level_vol(
                a,
                rets,
                i,
                equity_engine_weights=equity_engine_weights,
                commodity_engine_weights=commodity_engine_weights,
            )
            for a in w
        }
        ...
        rc = {a: v / total_rc for a, v in raw_rc.items()}
        max_asset = max(rc, key=rc.get)
        max_asset_rc = rc[max_asset]

        diagnostics["iterations"].append(
            {
                "weights": {k: float(v) for k, v in w.items()},
                "vols": {k: float(v) for k, v in vols.items()},
                "risk_contributions": {k: float(v) for k, v in rc.items()},
                "max_asset": max_asset,
                "max_asset_rc": float(max_asset_rc),
            }
        )

        if max_asset_rc <= max_rc:
            break

        w[max_asset] *= RISK_BRAKE_REDUCTION
        w = normalize_weights(w)

    diagnostics["after_weights"] = {k: float(v) for k, v in w.items()}

    return w, diagnostics


def build_v6_weights(
    prices,
    rets,
    i,
    gics_buckets,
    ...
    equity_factor_max=DEFAULT_EQUITY_FACTOR_MAX,
):
    equity_engine_weights, equity_diag = compute_equity_engine_weights(
        prices,
        i,
        gics_buckets,
        factor_max=equity_factor_max,
    )

    commodity_engine_weights, commodity_diag = compute_commodity_engine_weights(
        prices,
        i,
        commodity_buckets,
    )
    ...
    configured_assets = [
        ...
        "managed_futures",
        "gold",
        COMMODITY_ENGINE_ASSET,
        "cash",
    ]

    if include_bitcoin:
        configured_assets.append("bitcoin")

    available_assets = []

    for asset in configured_assets:
        if asset == EQUITY_ENGINE_ASSET:
            if equity_engine_weights and all(
                has_lookback(prices, a, i) for a in equity_engine_weights
            ):
                available_assets.append(asset)

        elif asset == COMMODITY_ENGINE_ASSET:
            if commodity_engine_weights and all(
                has_lookback(prices, a, i) for a in commodity_engine_weights
            ):
                available_assets.append(asset)
    ...
    rankable_assets = [
        a for a in available_assets if a != RISK_FREE_ASSET
    ]
    ...
    for asset in rankable_assets:
        if asset == EQUITY_ENGINE_ASSET:
            asset_scores[asset] = score_from_weights(
                prices,
                i,
                equity_engine_weights,
            )
        elif asset == COMMODITY_ENGINE_ASSET:
            asset_scores[asset] = score_from_weights(
                prices,
    ...
    for a in rankable_assets:
        s = asset_scores.get(a, np.nan)
        if pd.notna(s) and pd.notna(benchmark_score):
            rel_scores[a] = max(float(s - benchmark_score), 0.0)
        else:
            rel_scores[a] = 0.0

    power_scores = {
        a: v**SCORE_POWER
        for a, v in rel_scores.items()
        if v > 0
    }

    if sum(power_scores.values()) > 0:
        total_power = sum(power_scores.values())
        w_step = {
            a: v / total_power
            for a, v in power_scores.items()
        }
        fallback_used = False
        fallback_reason = None
    else:
        w_step = build_benchmark_asset_weights()
        fallback_used = True
        fallback_reason = "no_relative_outperformance"

    w_step, risk_brake_diag = apply_risk_contribution_brake(
        w_step,
        rets,
        i,
        equity_engine_weights=equity_engine_weights,
    ...
    caps = {
        COMMODITY_ENGINE_ASSET: MAX_TOTAL_COMMODITIES,
    }
    ...
    if (
        pd.notna(portfolio_score)
        and pd.notna(benchmark_score)
        and portfolio_score <= benchmark_score
    ...
    for asset, weight in w_step.items():
        if asset == EQUITY_ENGINE_ASSET:
            for inner_asset, inner_weight in equity_engine_weights.items():
                current_weights[inner_asset] = (
                    current_weights.get(inner_asset, 0.0)
                    + weight * inner_weight
                )
            ...
            current_weights[asset] = (
                current_weights.get(asset, 0.0) + weight
            )
    ...
    diagnostics = {
        ...
        "relative_scores": {k: float(v) for k, v in rel_scores.items()},
        "power_scores": {k: float(v) for k, v in power_scores.items()},
        "raw_asset_ranking": raw_ranking,
        "selected_assets": sorted(
            [k for k, v in w_step.items() if v > 0],
            key=lambda k: w_step[k],
            reverse=True,
        ),
        "equity_engine": equity_diag,
        "commodity_engine": commodity_diag,
        "risk_brake": risk_brake_diag,
        "asset_level_weights": {k: float(v) for k, v in w_step.items()},
        "asset_level_portfolio_score": None
        if pd.isna(weighted_score(w_step, asset_scores))
        else float(weighted_score(w_step, asset_scores)),
        "fallback_used": bool(fallback_used),
        "fallback_reason": fallback_reason,
        "weighting_method": "v6_relative_outperformance_score_power_plus_risk_contribution_brake",
...
def run(prices, rets, settings, gics_buckets, commodity_buckets):
    ...
    for i in range(start, len(prices) - 1):
        w, diagnostics = build_v6_weights(
            prices,
            rets,
            i,
            gics_buckets,
        ...
        turnover = float(
            (w - prev_w.reindex(w.index).fillna(0.0)).abs().sum()
        )
...
def get_stats(nav, label):
    ...
    cagr = (
        (nav.iloc[-1] / nav.iloc[0]) ** (1 / years) - 1
        if years > 0
        else 0.0
    )
    ...
    sharpe = (
        (rets.mean() / rets.std()) * math.sqrt(12)
...
def backtest_v6():
    try:
        ...
        settings = {
            ...
            "equity_factor_max": clamp_factor_max(
                data.get("equity_factor_max", DEFAULT_EQUITY_FACTOR_MAX)
            ),
            "sector_proxy_set": resolve_sector_proxy_set(
                data.get("sector_proxy_set", DEFAULT_SECTOR_PROXY_SET)
            ),
        }

        prices, rets, gics_buckets, commodity_buckets = load_data(
            settings["include_bitcoin"],
            settings["sector_proxy_set"],
        )
        ...
        availability = compute_availability(
            prices,
            assets,
            list(gics_buckets.keys()) + list(commodity_buckets.keys()),
        )
        ...
        proxies["managed_futures"] = (
            "data/SG_TRD_IDX.TXT"
            if SG_TREND_FILE.exists()
            else DEFAULT_TICKERS["managed_futures"]
        )
        ...
        return jsonify(
            {
                "summary": [
                    get_stats(nav.loc[idx], "Portfolio V6"),
                ...
                "history": history,
                "meta": {
                    "version": "v6_relative_score_risk_brake",
                    "include_bitcoin": settings["include_bitcoin"],
                    ...
                    "score_power": SCORE_POWER,
                    "max_risk_contribution": MAX_RISK_CONTRIBUTION,
                    "risk_brake_reduction": RISK_BRAKE_REDUCTION,
                    ...
                    "ranking_method": "relative score vs 70/30; weight proportional to positive relative score^power",
                    "weighting_method": "score_power_weighting_plus_risk_contribution_brake",
                    "benchmark_fallback_mode": BENCHMARK_FALLBACK_MODE,
                    "managed_futures_source": "local_sg_trend_file"
                    if SG_TREND_FILE.exists()
                    else "yfinance_dbmf",
