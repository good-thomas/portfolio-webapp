...
def compute_blended_weights(top_items, item_scores, benchmark_score, total_weight, power=EQUITY_ENGINE_EDGE_POWER):
    ...
    if power_sum <= 0:
        ...
        edge_weights = {b: total_weight * edge_power_scores[b] / power_sum for b in edge_power_scores}
...
def compute_equity_engine_weights(prices, i, gics_buckets, factor_max=DEFAULT_EQUITY_FACTOR_MAX):
    ...
    if not top_buckets or pd.isna(equity_score):
        diagnostics = {
            ...
            "factor_weight": 0.0,
            "base_weight": 1.0,
            "avg_relative_edge": 0.0,
            "max_relative_edge": 0.0,
    ...
    sector_weights, equal_weights, edge_weights, edges, edge_power_scores = compute_blended_weights(
        top_items=top_buckets,
        item_scores=bucket_scores,
        benchmark_score=equity_score,
        total_weight=factor_weight,
        power=EQUITY_ENGINE_EDGE_POWER,
    )

    internal_weights = {"equities": base_weight}
    internal_weights.update(sector_weights)
    total = sum(internal_weights.values())
    if total <= 0:
        internal_weights = {"equities": 1.0}
    else:
        internal_weights = {k: v / total for k, v in internal_weights.items()}
...
def build_v5_weights(prices, i, gics_buckets, include_bitcoin=True, equity_factor_max=DEFAULT_EQUITY_FACTOR_MAX):
    ...
    for asset in configured_assets:
        if asset == EQUITY_ENGINE_ASSET:
            if all(has_lookback(prices, a, i) for a in equity_engine_weights):
...
def run(prices, rets, settings, gics_buckets):
    ...
    for i in range(start, len(prices) - 1):
        w, diagnostics = build_v5_weights(
            prices,
            i,
            gics_buckets,
...
@app.route("/api/backtest_v5", methods=["POST"])
def backtest_v5():
    try:
